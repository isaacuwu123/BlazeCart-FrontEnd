            ...
            >
                <div class="flex items-center space-x-2 sm:space-x-4">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        >
                            Gestión de Documentos de Inversionistas
                        </h1>
                        <p
                            class="mt-1 text-xs truncate transition-colors duration-200 sm:text-sm lg:text-base"
                            :class="isDarkMode ? 'text-gray-300' : 'text-gray-600'"
                        >
                            Administra los documentos asociados a los inversionistas
                        </p>
                    </div>
                </div>
                <button
        ...
        >
            <div class="flex flex-col space-y-3 sm:space-y-0 sm:flex-row sm:space-x-4">
                ...
                >
                    ...
                    <option value="contrato_inversion">Contrato de Inversión</option>
                    <option value="comprobante_pago">Comprobante de Pago</option>
                    <option value="identificacion">Identificación</option>
                    <option value="estado_financiero">Estado Financiero</option>
                    <option value="certificado_fiscal">Certificado Fiscal</option>
                    <option value="otros">Otros</option>
                </select>
                <select
                    v-model="selectedAprobado"
                    class="px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg border focus:ring-2 focus:ring-yellow-400 focus:border-transparent text-sm sm:text-base min-w-0 sm:min-w-[160px] transition-colors duration-200"
                    :class="
                        isDarkMode
                            ? 'bg-gray-700 border-gray-600 text-white'
                        ...
                        >
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div
                                    ...
                                    :class="isDarkMode ? 'text-white' : 'text-gray-900'"
                                >
                                    {{ document.investor?.user?.general_data?.nombre || 'N/A' }}
                                    {{ document.investor?.user?.general_data?.apellido || '' }}
                                </div>
                                <div
                                    class="text-sm"
                                    :class="isDarkMode ? 'text-gray-300' : 'text-gray-500'"
                                >
                                    {{ document.investor?.user?.email || 'N/A' }}
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    ...
                                    "
                                >
                                    {{ document.aprobado ? 'Aprobado' : 'No Aprobado' }}
                                </span>
                            </td>
                            ...
                            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4">
                                <div class="sm:col-span-2">
                                    ...
                                    >
                                        ...
                                        <option
                                            ...
                                            :value="investor.id"
                                        >
                                            {{ investor.user?.general_data?.nombre || 'N/A' }}
                                            {{ investor.user?.general_data?.apellido || '' }} ({{
                                                investor.user?.email || 'N/A'
                                            }})
                                        </option>
                                    </select>
                                    <p
                                        v-if="formErrors.investor_id"
                                        class="mt-1 text-xs text-red-500"
                                    >
                                        {{ formErrors.investor_id }}
                                    </p>
                                </div>
                                <div>
                                    <label
                                ...
                                <div>
                                    ...
                                    >
                                        ...
                                        <option value="contrato_inversion">
                                            Contrato de Inversión
                                        </option>
                                        <option value="comprobante_pago">
                                            Comprobante de Pago
                                        </option>
                                        <option value="identificacion">Identificación</option>
                                        <option value="estado_financiero">Estado Financiero</option>
                                        <option value="certificado_fiscal">
                                            Certificado Fiscal
                                        </option>
                                        <option value="otros">Otros</option>
                                    </select>
                                ...
                                <div>
                                    ...
                                    <p v-if="formErrors.aprobado" class="mt-1 text-xs text-red-500">
                                        {{ formErrors.aprobado }}
                                    </p>
                                </div>
                                <div class="sm:col-span-2">
                                    ...
                                    <div v-if="filePreview" class="mt-2">
                                        <img
                                            v-if="isImage(formData.archivo || filePreview)"
                                            :src="filePreview"
...
/* import {
    ...
    indexInvestorDocuments,
    showInvestorDocument,
    updateInvestorDocument,
} from '@/services/InvestorDocumentService';
import { indexInvestors } from '@/services/InvestorService'; */
import {
    AlertCircleIcon,
    CalendarIcon,
    DownloadIcon,
...
const selectedAprobado = ref('');
const posicion = ref(null);
const formData = ref({
    investor_id: '',
    tipo: 'contrato_inversion',
    nombre: '',
    archivo: null,
    fecha_vencimiento: '',
    aprobado: 'false',
...
const filteredDocuments = computed(() => {
    ...
    if (searchQuery.value) {
        ...
        filtered = filtered.filter(
            doc =>
                ...
                (doc.investor?.user?.general_data?.nombre || '').toLowerCase().includes(search) ||
                (doc.investor?.user?.general_data?.apellido || '').toLowerCase().includes(search) ||
                (doc.investor?.user?.email || '').toLowerCase().includes(search)
        );
    }
    ...
    if (selectedAprobado.value !== '') {
        filtered = filtered.filter(doc => doc.aprobado.toString() === selectedAprobado.value);
    }
...
const paginatedDocuments = computed(() => {
    ...
    totalPages.value = Math.ceil(filteredDocuments.value.length / perPage.value);
...
const listDocuments = async () => {
    try {
        showLoading();
        const { data } = await indexInvestorDocuments();
...
const listInvestors = async () => {
    try {
        const { data } = await indexInvestors();
        investors.value = data.datos;
    } catch (error) {
        console.error('Error listing investors:', error);
        showError('Error', 'No se pudo cargar la lista de inversionistas.');
    }
};
...
const handleFileUpload = event => {
    ...
    if (file) {
        ...
        if (!ALLOWED_FILE_TYPES.includes(file.type)) {
            formErrors.value.archivo = 'Formato no válido. Use JPEG, PNG, JPG, GIF o PDF.';
            formData.value.archivo = null;
            filePreview.value = null;
            fileInput.value = '';
            return;
...
const showDocumentById = async documentId => {
    try {
        showLoading();
        const { data } = await showInvestorDocument(documentId);
        formData.value = {
            investor_id: data.datos.investor_id || '',
            tipo: data.datos.tipo || 'contrato_inversion',
            nombre: data.datos.nombre || '',
            archivo: null,
            fecha_vencimiento: data.datos.fecha_vencimiento
                ? data.datos.fecha_vencimiento.split('T')[0]
                : '',
            aprobado: data.datos.aprobado.toString(),
            observaciones: data.datos.observaciones || '',
        };
        filePreview.value = isImage(data.datos.archivo)
            ? data.datos.archivo_url
            : data.datos.archivo;
...
const validateFormData = () => {
    ...
    if (!formData.value.investor_id) {
        formErrors.value.investor_id = 'Debe seleccionar un inversionista.';
        isValid = false;
    }
...
const handleSaveDocument = async () => {
    try {
        ...
        try {
            data.append('investor_id', formData.value.investor_id || '');
            ...
            data.append('aprobado', formData.value.aprobado);
        ...
        if (posicion.value) {
            await updateInvestorDocument(posicion.value, data);
            showSuccess('Éxito', 'Documento actualizado correctamente.');
        } else {
            await createInvestorDocument(data);
...
const confirmDelete = async document => {
    ...
    if (confirmed) {
        try {
            showLoading();
            await deleteInvestorDocument(document.id);
...
const downloadDocument = async documentId => {
    try {
        const response = await downloadInvestorDocument(documentId);
...
// Watchers
watch([searchQuery, selectedDocumentType, selectedAprobado], () => {
    currentPage.value = 1;
});
