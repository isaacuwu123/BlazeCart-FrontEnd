            ...
            >
                <div class="flex items-center space-x-2 sm:space-x-4">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        >
                            Administra los documentos asociados a los vehículos
                        </p>
                    </div>
                </div>
                <button
            ...
            <div class="flex flex-col space-y-3 sm:space-y-0 sm:flex-row sm:space-x-4">
                <div class="relative flex-1">
                    ...
                    <input
                        v-model="searchQuery"
                        type="text"
                        placeholder="Buscar por nombre, VIN, placa, marca o modelo..."
                ...
                >
                    ...
                    <option value="certificado_emisiones">Certificado de Emisiones</option>
                        ...
                        >
                                ...
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 w-10 h-10">
                                        <img
                                            ...
                                            :src="document.url_previsualizacion || document.archivo"
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                >
                                    {{
                                        document.vehicle
                                            ? `${document.vehicle.marca} ${document.vehicle.modelo}`
                                            : 'N/A'
                                    }}
                                </div>
                                <div
                                    ...
                                    :class="isDarkMode ? 'text-gray-300' : 'text-gray-500'"
                                >
                                    {{ document.vehicle?.numero_placa || 'N/A' }}
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div
                            ...
                            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4">
                                    ...
                                    <input
                                        ...
                                        :class="
                                            isDarkMode
                                                ...
                                                : 'bg-white border-gray-300 text-gray-900 placeholder-gray-500'
                                        "
                                        placeholder="Registro Vehicular"
                                    />
                                ...
                                <div>
                                    ...
                                    >
                                        ...
                                        <option value="certificado_emisiones">
    ...
    if (searchQuery.value) {
        ...
        filtered = filtered.filter(
            doc =>
                ...
                (doc.vehicle?.numero_vin || '').toLowerCase().includes(search) ||
                (doc.vehicle?.numero_placa || '').toLowerCase().includes(search) ||
                (doc.vehicle?.marca || '').toLowerCase().includes(search) ||
                (doc.vehicle?.modelo || '').toLowerCase().includes(search)
        );
    }
...
const listDocuments = async () => {
    try {
        showLoading();
        const { data } = await indexVehicleDocuments();
...
const showDocumentById = async documentId => {
    try {
        showLoading();
        const { data } = await showVehicleDocument(documentId);
        console.log(data);
        formData.value = {
            vehicle_id: data.datos.vehicle_id || '',
            tipo: data.datos.tipo || 'registro_vehicular',
            nombre: data.datos.nombre || '',
            archivo: null,
            fecha_vencimiento: data.datos.fecha_vencimiento
                ? data.datos.fecha_vencimiento.split('T')[0]
                : '',
            aprobado: data.datos.aprobado === 1,
            observaciones: data.datos.observaciones || '',
        };

        filePreview.value = isImage(data.datos.archivo)
            ? data.datos.url_previsualizacion || data.datos.archivo
            : data.datos.archivo;
...
const handleSaveDocument = async () => {
    try {
        ...
        if (posicion.value) {
            await updateVehicleDocument(posicion.value, data);
...
const confirmDelete = async document => {
    try {
        console.log('Iniciando proceso de eliminación para el documento:', document.id);
        ...
        console.log('Procediendo con la eliminación del documento:', document.id);
        showLoading();
        ...
        showError('Error', error.response?.data?.message || 'No se pudo eliminar el documento.');
    } finally {
        closeLoading();
    }
};
...
const downloadDocument = async documentId => {
    try {
        const response = await downloadVehicleDocument(documentId);
