    ...
    <div class="min-h-screen p-4 transition-colors duration-300 bg-gray-50 dark:bg-gray-950 sm:p-6">
        <div class="mx-auto max-w-7xl">
            ...
            >
                ...
                <div v-if="egresosPorCategoria?.resumen_global" class="mt-2 text-center">
                ...
                <div class="flex justify-center mt-4 space-x-4">
                    ...
                    <button
                        @click="exportarPDF"
                        :disabled="cargando || !egresosPorCategoria?.resumen_global"
                        class="btn"
                    >
            ...
            >
                <!-- Contenido de Egresos por Categoría -->
                ...
                <!-- Egresos por Categoría -->
                <div
                    v-else-if="egresosPorCategoria?.resumen_global"
                ...
                >
                    ...
                    <div class="flex flex-col h-full chart-card">
                        <h4 class="chart-title">📊 Distribución de Egresos por Categoría</h4>
                    ...
                    <div class="grid grid-cols-1 gap-6 xl:grid-cols-3">
                        ...
                        <div class="flex flex-col gap-6 xl:col-span-2">
                                ...
                                >
                                    ...
                                    >
                                        <div class="font-medium text-gray-900 dark:text-gray-100">
                                            {{ getNombreCategoria(categoria.categoria_nombre) }}
                                        </div>
                                        <div class="font-semibold text-red-600 dark:text-red-400">
                                            {{ formatCurrency(categoria.total_categoria) }}
                                        </div>
                                        <div class="text-gray-600 dark:text-gray-400">
                                            {{ categoria.cantidad_transacciones }}
                                        </div>
                                        <div class="text-gray-600 dark:text-gray-400">
                                            {{ formatCurrency(categoria.promedio_egreso) }}
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div v-if="egresosPorCategoria.estadisticas_adicionales" class="mt-6">
                            ...
                            <div class="space-y-3">
                                ...
                                >
                                    ...
                                    >
                                        {{
                                            formatCurrency(
                                                egresosPorCategoria.resumen_global?.total_egresos ||
                                                    0
                                            )
                                        }}
                                    </span>
                ...
                <!-- Empty state -->
                <div
                    v-else-if="!cargando && !error && !egresosPorCategoria?.resumen_global"
                ...
                >
                    <div class="text-gray-500 dark:text-gray-400">
                        ...
                        <p>Elige los filtros para generar tu reporte de egresos por categoría</p>
                    </div>
                </div>
            </div>
        </div>
...
// Obtener nombre de categoría (cambia "Dividendo" por "Comisiones")
const getNombreCategoria = nombre => {
    if (!nombre) return '';
    return nombre === 'Dividendo' ? 'Comisiones' : nombre;
};
...
const exportarExcel = async () => {
    ...
    try {
        ...
        link.href = url;
        link.download = `Egresos_por_Categoria_${formatDate(filtros.value.fecha_final)}.xlsx`;
...
const exportarPDF = async () => {
    ...
    try {
        ...
        const link = document.createElement('a');
        link.href = url;
        link.download = `Egresos_por_Categoria_${formatDate(filtros.value.fecha_final)}.pdf`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        window.URL.revokeObjectURL(url);
...
// Datos para gráfico de egresos por categoría (barras horizontales)
const egresosPorCategoriaChartData = computed(() => {
    if (!egresosPorCategoria.value?.categorias) return { labels: [], datasets: [] };

    const categorias = egresosPorCategoria.value.categorias.map(item =>
        getNombreCategoria(item.categoria_nombre)
    );
    const valores = egresosPorCategoria.value.categorias.map(item => {
        // Asegurarse de que sea un número
        return typeof item.total_categoria === 'number'
            ? item.total_categoria
            : parseFloat(item.total_categoria) || 0;
    });
