<template>
    ...
    >
            ...
            >
                <div class="flex items-center space-x-2 sm:space-x-4">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        >
                            Administra los estados de transacción del sistema
                        </p>
                    </div>
                </div>
                <button
            ...
            <div class="flex flex-col space-y-3 sm:space-y-0 sm:flex-row sm:space-x-4">
                <div class="relative flex-1">
                    ...
                    <input
                        v-model="searchQuery"
                        type="text"
                        placeholder="Buscar estados de transacción..."
        ...
        <div class="grid grid-cols-1 gap-4 mb-6 sm:grid-cols-3">
            ...
            >
                ...
                <p class="text-xl font-bold" :class="isDarkMode ? 'text-white' : 'text-gray-900'">
                    {{ filteredTransactionStates.length }}
                </p>
            </div>
            <div
                class="p-4 transition-colors duration-200 shadow-lg rounded-xl"
            ...
            >
                ...
                >
                    {{ filteredTransactionStates.filter(s => s.estado).length }}
                </p>
            </div>
            <div
                class="p-4 transition-colors duration-200 shadow-lg rounded-xl"
        ...
        >
            <div class="overflow-x-auto">
                ...
                >
                    ...
                    >
                        <tr
                            v-for="state in paginatedTransactionStates"
                            :key="state.id"
                        ...
                        >
                            ...
                            <td class="px-6 py-4 text-sm font-medium whitespace-nowrap">
                                <div class="flex items-center space-x-2">
                                    ...
                                    <button
                                        @click="toggleTransactionStateStatus(state)"
                                        :class="
                                            state.estado
                                        ...
                                        :title="state.estado ? 'Desactivar' : 'Activar'"
                                    >
                                        <PowerIcon v-if="state.estado" class="w-4 h-4" />
        ...
        <!-- Empty State -->
        <div v-if="paginatedTransactionStates.length === 0" class="py-8 text-center sm:py-12">
            <div
            ...
            >
                ...
                <h3
                    ...
                    :class="isDarkMode ? 'text-white' : 'text-gray-900'"
                >
                    No se encontraron estados de transacción
                </h3>
                <p
                    class="text-sm transition-colors duration-200 sm:text-base"
                    :class="isDarkMode ? 'text-gray-400' : 'text-gray-500'"
                >
                    Intenta con diferentes filtros o crea un nuevo estado.
                </p>
            </div>
        </div>
        <!-- Edit/Create Modal -->
        ...
        >
            ...
            >
                ...
                >
                    <div class="flex items-center justify-between">
                        <h2 class="text-lg font-bold text-white sm:text-xl">
                            {{ posicion ? 'Editar' : 'Crear' }} Estado de Transacción
                        </h2>
                        <button
                ...
                <div class="p-4 sm:p-6">
                    <form
                        @submit.prevent="handleSaveTransactionState"
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                ...
                                Información del Estado de Transacción
                            </h3>
                                    ...
                                    <input
                                        ...
                                        :class="
                                            isDarkMode
                                                ...
                                                : 'bg-white border-gray-300 text-gray-900'
                                        "
                                        placeholder="Nombre del estado de transacción"
                                    />
                                    ...
                                    <textarea
                                        ...
                                        :class="
                                            isDarkMode
                                                ...
                                                : 'bg-white border-gray-300 text-gray-900'
                                        "
                                        placeholder="Descripción del estado de transacción"
                                        rows="3"
                                    ></textarea>
                                    <p
                        ...
                        >
                            ...
                            >
                                ...
                                <span v-else>{{ posicion ? 'Actualizar' : 'Crear' }} Estado</span>
                            </button>
                        </div>
                    </form>
                </div>
...
import {
    activateTransactionState,
    deactivateTransactionState,
    indexTransactionStates,
    showTransactionState,
    storeTransactionState,
    updateTransactionState,
} from '@/services/TransactionStateService';
import {
    EditIcon,
    PlusIcon,
    PowerIcon,
...
const filteredTransactionStates = computed(() => {
    return transactionStates.value.filter(state => {
        const matchesSearch = state.nombre.toLowerCase().includes(searchQuery.value.toLowerCase());
        const matchesStatus =
            selectedStatus.value === '' || state.estado.toString() === selectedStatus.value;
        return matchesSearch && matchesStatus;
    });
});
const paginatedTransactionStates = computed(() => {
    const start = (currentPage.value - 1) * perPage.value;
    const end = start + perPage.value;
    return filteredTransactionStates.value.slice(start, end);
});
const totalPages = computed(() =>
    Math.ceil(filteredTransactionStates.value.length / perPage.value)
);
// Methods
const listTransactionStates = async () => {
    try {
        showLoading();
        const { data } = await indexTransactionStates();
        console.log(data);
        transactionStates.value = data.datos || [];
    } catch (error) {
        console.error('Error loading transaction states:', error);
...
const openEditModal = async state => {
    try {
        showLoading();
        const { data } = await showTransactionState(state.id);
        form.value = { ...data.datos };
        posicion.value = state.id;
        isEditModalOpen.value = true;
    } catch (error) {
        showError('Error', 'No se pudo cargar el estado de transacción.');
    } finally {
        closeLoading();
    }
};
...
const handleSaveTransactionState = async () => {
    if (!validateForm()) return;
    try {
        showLoading();
        isLoading.value = true;
        if (posicion.value) {
            await updateTransactionState(posicion.value, form.value);
            showSuccess('Éxito', 'Estado de transacción actualizado.');
        } else {
            await storeTransactionState(form.value);
            showSuccess('Éxito', 'Estado de transacción creado.');
        }
        await listTransactionStates();
        closeEditModal();
    } catch (error) {
        formErrors.value = error.response?.data?.errors || {};
        showError(
            'Error',
            `No se pudo ${posicion.value ? 'actualizar' : 'crear'} el estado de transacción.`
        );
    } finally {
...
const toggleTransactionStateStatus = async state => {
    const action = state.estado ? 'desactivar' : 'activar';
    const confirmed = await showConfirm(
        'Confirmar',
        `¿${action.charAt(0).toUpperCase() + action.slice(1)} el estado "${state.nombre}"?`
    );
    if (confirmed) {
        try {
            showLoading();
            if (state.estado) {
                await deactivateTransactionState(state.id);
            } else {
                await activateTransactionState(state.id);
            }
            showSuccess('Éxito', `Estado de transacción ${action}do.`);
            await listTransactionStates();
        } catch (error) {
            showError('Error', `No se pudo ${action} el estado de transacción.`);
        } finally {
            closeLoading();
        }
    }
